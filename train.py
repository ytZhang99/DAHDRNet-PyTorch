    ...
    def train(self):
        ...
        for ep in range(self.num_epochs):
            ...
            for batch_idx, batch_data in enumerate(self.train_loader):
                ...
                label_rc = range_compressor_tensor(label)
                label_rc = torch.clamp(label_rc, 0., 1.)

                torch.cuda.synchronize()
                start_time = time.time()

                pred = self.model(batch_ldr0, batch_ldr1, batch_ldr2)
                pred_rc = range_compressor_tensor(pred)
                pred_rc = torch.clamp(pred_rc, 0., 1.)
                
                self.optimizer.zero_grad()
                loss_1 = self.pixel_loss(pred_rc, label_rc)
                loss_2 = self.perceptual_loss(pred, label)
                loss_3 = self.tv_loss(pred)
                loss = loss_1 + 0.0001 * loss_2 + 0.01 * loss_3
                loss.backward()
                self.optimizer.step()
